...
public class StudentLogin extends Activity{
	...
	public void  initView(){
		final SharedPreferences sPreferences=getSharedPreferences("studentuser",Context.MODE_PRIVATE);
		...
		loginButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				//获得Preferences保存的对象
				String username=sPreferences.getString("susername","");
				if(loginButton.getText().toString().equals("登录")){
					...
					if(!usernameEdit.getText().toString().equals("")&&!password.getText().toString().equals("")){//当前用户不等于空才可以登录
						...
						utils.send(HttpMethod.POST, UrlUtils.url+"/StudentCheckAccount", params, new RequestCallBack<String>() {

							@Override
							public void onFailure(HttpException arg0,
									String arg1) {
								Toast.makeText(StudentLogin.this, "请求网络数据失败", Toast.LENGTH_LONG).show();
							...
							public void onSuccess(ResponseInfo<String> responseInfo) {
								...
								if(flag.equals("true")){
									...
									editor.putString("susername",usernameEdit.getText().toString());
									editor.commit();
									Toast.makeText(StudentLogin.this, "登录成功", Toast.LENGTH_LONG).show();
									Intent intent=new Intent(StudentLogin.this,StudentHomeActivity.class);
									startActivity(intent);
									finish();
								}else{
									Toast.makeText(StudentLogin.this, "亲，请检查用户名或密码是否正确哦", Toast.LENGTH_LONG).show();
						...
						Toast.makeText(StudentLogin.this, "您已登录，不能重复登录", Toast.LENGTH_LONG).show();
				
					}
					}else{
						Toast.makeText(StudentLogin.this, "用户名或密码不能为空", Toast.LENGTH_LONG).show();
					}
